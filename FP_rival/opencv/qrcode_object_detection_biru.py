...
while True:
    ...
    if retval:
        for i, points_set in enumerate(points):
            if points_set is not None and decoded_info[i] == "PENJARA BIRU":
...
def detect_blue_qr(frame):
    ...
    # Detect QR code
    retval, decoded_info, points, _ = qr_detector.detectAndDecodeMulti(adjusted)
    ...
    if retval:
        for i, points_set in enumerate(points):
            if points_set is not None and decoded_info[i] == "PENJARA BIRU":
                # Calculate center point
                center_x = int(np.mean(points_set[:, 0]))
                frame_width = adjusted.shape[1]
                
                # Check if QR code is in center area
                if frame_width//3 < center_x < (frame_width * 2)//3:
                    return True
